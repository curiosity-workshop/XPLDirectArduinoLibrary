...
{
//	streamPtr->print("Device name: ");
//	streamPtr->println(devicename);

//	streamPtr->begin(XPLDIRECT_BAUDRATE);
...
int XPLDirect::xloop(void)
{
	...
	if (!_allDataRefsRegistered) return _connectionStatus;
			...
			switch (_dataRefs[i]->dataRefVARType)
			{
				case XPL_DATATYPE_INT:
				{
						...
						if ((*(long int*)_dataRefs[i]->latestValue != _dataRefs[i]->lastSentIntValue
							&& millis() - _dataRefs[i]->lastUpdateTime > _dataRefs[i]->updateRate)  || _dataRefs[i]->forceUpdate)
						{
				...
				case XPL_DATATYPE_FLOAT:
				{
					...
					if (_dataRefs[i]->divider)  *(float*)_dataRefs[i]->latestValue = ((int)(*(float*)_dataRefs[i]->latestValue / _dataRefs[i]->divider) * _dataRefs[i]->divider);
							...
							&& (millis() - _dataRefs[i]->lastUpdateTime) > _dataRefs[i]->updateRate) || _dataRefs[i]->forceUpdate)
						{
							...
							//streamPtr->println(_dataRefs[i]->lastSentFloatValue);
	...
	for (i = 0; i < _commandsCount; i++)
	{
		if (_commands[i]->commandHandle >= 0 && millis() - _commands[i]->lastUpdateTime > XPL_COMMAND_MAX_UPDATE_RATE_MILLIS)
		{
			...
			if (_commands[i]->pin >= 0)			// process if assigned pin
			{
				...
				//streamPtr->print("command: "); streamPtr->print(i); streamPtr->print(" pinstatus: "); streamPtr->print(pinStatus); streamPtr->print(" last sent: "); streamPtr->println(_commands[i]->lastSentValue);
			...
			else								// process for normal variables
			{
				...
				if (_commands[i]->latestValue != NULL && *(int *)_commands[i]->latestValue != _commands[i]->lastSentValue)
				{
					...
					if (*_commands[i]->latestValue > 0)	_sendPacketVoid(XPLCMD_COMMANDEND, _commands[i]->commandHandle);
					else								_sendPacketVoid(XPLCMD_COMMANDSTART, _commands[i]->commandHandle);
...
{
	if (!_commands[commandHandle]) return -1;
...
void XPLDirect::datarefRead(int handle, long int * value)
{
...
void XPLDirect::datarefRead(int handle, float * value)
{
	if (_dataRefs[handle]->dataRefHandle >= 0 && (_dataRefs[handle]->dataRefRWType == XPL_READ || _dataRefs[handle]->dataRefRWType == XPL_READWRITE))
...
void XPLDirect::datarefWrite(int handle, long int value)
{
...
void XPLDirect::datarefWrite(int handle, float value)
{
	if (_dataRefs[handle]->dataRefHandle >= 0 && (_dataRefs[handle]->dataRefRWType == XPL_READ || _dataRefs[handle]->dataRefRWType == XPL_READWRITE))
...
void XPLDirect::_processSerial()
{
	...
	while (streamPtr->available() && _receiveBuffer[0] != XPLDIRECT_PACKETHEADER)  _receiveBuffer[0] = (char)streamPtr->read();

	if (_receiveBuffer[0] != XPLDIRECT_PACKETHEADER) return;
	...
	//streamPtr->print(_receiveBuffer);
...
/*
	...
	byte validFrame = false;

	while (streamPtr->available() && _receiveBuffer[0] != XPLDIRECT_PACKETHEADER)  _receiveBuffer[0] = (char)streamPtr->read();   // search for header
	
	if (_receiveBuffer[0] != XPLDIRECT_PACKETHEADER) return;

	_receiveBufferBytesReceived = 1;
	while (!validFrame)
	{

		_receiveBufferBytesReceived += streamPtr->readBytesUntil(XPLDIRECT_PACKETTRAILER, (char*)&_receiveBuffer[_receiveBufferBytesReceived], XPLMAX_PACKETSIZE - 1);

		if (_receiveBufferBytesReceived == 1)
		{
			_receiveBuffer[0] = 0;
			return;
		}
		...
		{
			if (streamPtr->peek() != XPLDIRECT_PACKETHEADER) validFrame = false;
//			streamPtr->print(streamPtr->available());
		}
	}
	// valid frame found
...
//	streamPtr->print(_receiveBuffer);
//	streamPtr->print(" bytes Received:");
//	streamPtr->println(_receiveBufferBytesReceived);
...
{
 /*   streamPtr->print("valid frame discovered, length = ");
	streamPtr->print(rxbuffer.length());
	streamPtr->print(", here it is: ");
    streamPtr->print(rxbuffer);
	streamPtr->println();
*/
	int i;
	...
	switch (_receiveBuffer[1])
	{
		...
		{   
			...
			// streamPtr->print("looking through this many refs: "); streamPtr->print(_dataRefsCount); 
			...
			//	streamPtr->println(i);
				...
				{
					if (strncmp((char*)&_receiveBuffer[5], _dataRefs[i]->dataRefName, strlen(_dataRefs[i]->dataRefName)) == 0
						&& _dataRefs[i]->dataRefHandle == -1)
					{
	//	streamPtr->print("found dataref: "); streamPtr->println(_dataRefs[i]->dataRefName);

						_dataRefs[i]->dataRefHandle = _getHandleFromFrame(); // parse the refhandle	
						_dataRefs[i]->updatedFlag = true;
	//	streamPtr->print("it was assigned handle: "); streamPtr->println(_dataRefs[i]->dataRefHandle);
				...
				{
					//streamPtr->print("Comparing: "); streamPtr->print((char*)&_receiveBuffer[5]); streamPtr->print(" and:"); streamPtr->print(_dataRefs[i]->FdataRefName);
				
					if (strncmp_PF((char*)&_receiveBuffer[5], _dataRefs[i]->FdataRefName, strlen_PF(_dataRefs[i]->FdataRefName) )== 0
						&& _dataRefs[i]->dataRefHandle == -1)
					{
						//	streamPtr->print("found Fdataref: "); streamPtr->println(_dataRefs[i]->FdataRefName);
		...
		case XPLRESPONSE_COMMAND:
		{
			...
			{
				//streamPtr->println(rxbuffer.substring(5, rxbuffer.length() - 1));
				if (_commands[i]->commandName != NULL)
				{
					if (strncmp((char*)&_receiveBuffer[5], _commands[i]->commandName, strlen(_commands[i]->commandName)) == 0
						&& _commands[i]->commandHandle == -1)
					{
				//streamPtr->print("found dataref: "); streamPtr->println(_commands[i]->commandName);
						_commands[i]->commandHandle = _getHandleFromFrame();   // parse the refhandle	
				//streamPtr->print("it was assigned handle: "); streamPtr->println(_commands[i]->commandHandle);
				...
				{
					if (strncmp_PF((char*)&_receiveBuffer[5], _commands[i]->FcommandName, strlen_PF(_commands[i]->FcommandName)) == 0
						&& _commands[i]->commandHandle == -1)
					{
						_commands[i]->commandHandle = _getHandleFromFrame();   // parse the refhandle	
						i = _commandsCount;		// end checking
		...
		{   int packetSent = 0;
			...
			while  (!packetSent && i < _dataRefsCount && i < XPLDIRECT_MAXDATAREFS_ARDUINO)			// send dataref registrations first
			{
				...
				{// some boards cant do sprintf with floats so this is a workaround
					...
					//streamPtr->println(_sendBuffer);
					_transmitPacket();
					//streamPtr -> println("...packet sent");
					packetSent = 1;
		...
		case XPLCMD_DATAREFUPDATE :
		{
			...
			//streamPtr->println();
			//streamPtr->print("updating refhandle ");
			//streamPtr->println(refhandle);
			...
			for (int i = 0; i < _dataRefsCount; i++)
			{
				...
				{  
					if (_dataRefs[i]->dataRefVARType == XPL_DATATYPE_INT)
					{
						...
						/*streamPtr->println();
						streamPtr->print(" for handle ");
						streamPtr->print(refhandle);
						streamPtr->print(" I received a value of ");
						streamPtr->print(*_dataRefs[i].latestIntValue);
						streamPtr->print(" derived from string: ");
						streamPtr->println(rxbuffer.substring(5, 12));
						streamPtr->print(" It was assigned to the DataRef with name ");
						streamPtr->println(_dataRefs[i].dataRefName); */
...
void XPLDirect::_sendPacketString(int command, const char *str)			// for a string
{
...
//streamPtr->print("writing packet: ");						// debug stuff
//streamPtr->println(_sendBuffer);

	streamPtr->write(_sendBuffer);
	if (strlen(_sendBuffer) == 64) streamPtr->print(" ");			// apparantly a bug on some boards when we transmit exactly 64 bytes
...
int XPLDirect::_getPayloadFromFrame(char* value)			// Assuming receive buffer is holding a good frame
{
	...
	for (int i = 0; i < _receiveBufferBytesReceived - 6; i++)
		if (value[i] == 7) value[i] = XPLDIRECT_PACKETTRAILER;			//  How I deal with the possibility of the packet trailer being within a string
...
int XPLDirect::registerDataRef(const __FlashStringHelper* datarefName, int rwmode, unsigned int rate, float divider, long int* value)
{
	...
	if (_dataRefsCount >= XPLDIRECT_MAXDATAREFS_ARDUINO) return -1;				// Error
...
int XPLDirect::registerDataRef(const char* datarefName, int rwmode, unsigned int rate, float divider, long int* value)
{
	...
	if (_dataRefsCount >= XPLDIRECT_MAXDATAREFS_ARDUINO) return -1;
	_dataRefs[_dataRefsCount] = new _dataRefStructure;
...
int XPLDirect::registerCommand(const __FlashStringHelper* commandName, int* value)
{
	...
	if (_commandsCount >= XPLDIRECT_MAXCOMMANDS_ARDUINO) return -1;
